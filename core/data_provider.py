...
def get_earnings_history(ticker: str) -> pd.DataFrame:
    """
    Fetch quarterly EPS history (estimate vs actual) for a ticker.

    Returns:
        DataFrame sorted chronologically with DatetimeIndex and columns:
...
def get_recommendations(ticker: str) -> pd.DataFrame:
    """
    Fetch monthly analyst recommendations breakdown.

    Returns:
        DataFrame with columns: period, strongBuy, buy, hold, sell, strongSell.
        'period' values are '0m' (current month), '-1m', '-2m', '-3m', etc.
    ...
    try:
        ...
        logger.warning("Failed to fetch recommendations for %s: %s", ticker, e)
        return pd.DataFrame()


@st.cache_data(ttl=3600, show_spinner=False)
def get_upgrades_downgrades(ticker: str) -> pd.DataFrame:
    """
    Fetch analyst upgrades/downgrades history.

    Returns:
        DataFrame sorted most-recent-first with DatetimeIndex (GradeDate) and
        columns: Firm, ToGrade, FromGrade, Action, priceTargetAction,
        currentPriceTarget, priorPriceTarget.
        Empty DataFrame if unavailable.
    """
    import yfinance as yf

    logger.info("Fetching upgrades/downgrades for %s", ticker)
    try:
        stock = yf.Ticker(ticker)
        ...
        logger.warning("Failed to fetch upgrades/downgrades for %s: %s", ticker, e)
        return pd.DataFrame()
